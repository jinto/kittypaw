...
// channel with tenantID so every emitted Event can be routed to the right
// tenant's engine session by the TenantRouter.
func FromConfig(tenantID string, cfg core.ChannelConfig) (Channel, error) {
	switch cfg.ChannelType {
	case core.ChannelTelegram:
		if cfg.Token == "" {
			return nil, fmt.Errorf("telegram channel requires a bot token")
		}
		return NewTelegram(tenantID, cfg.Token), nil
	...
	case core.ChannelSlack:
		...
		return NewSlack(tenantID, cfg.Token, cfg.BindAddr), nil

	case core.ChannelDiscord:
		if cfg.Token == "" {
			return nil, fmt.Errorf("discord channel requires a bot token")
		}
		return NewDiscord(tenantID, cfg.Token), nil
