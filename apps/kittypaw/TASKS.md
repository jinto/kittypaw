...
### 🔴 Workspace Indexer v2 Hardening (Plan 27 follow-up)

Plan 27 (fsnotify live indexing, 8c45a4f) 이후 남은 3 개 silent failure 모드 제거.

- [ ] **Prefix delete**: 워치된 디렉토리 Remove/Rename 시 하위 파일 FTS row 일괄 삭제 (`store.DeleteWorkspaceFilesByPrefix`). 현재는 exact `abs_path` 1 건만 지워져 고아 row 축적.
- [ ] **Watch failure 가시성**: 서브디렉토리 `fsnotify.Add` 실패가 Debug 레벨에 묻혀 "건강해 보이지만 부분 서브트리가 죽은 상태" 발생. `slog.Warn` 승격 + `LiveIndexer.PartialFailures` 상태 노출.
- [ ] **Queue overflow 복구**: Linux `IN_Q_OVERFLOW` 이벤트 감지 시 해당 workspace 자동 full reindex 1 회 (debounce + 중복 방지).
