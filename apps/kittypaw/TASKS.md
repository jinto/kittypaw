...
### 🚧 진행 중: Workspace Indexer v2 Hardening (Plan 27 follow-up, 1·2 번들)

Plan: `.claude/plans/workspace-indexer-v2-hardening.md`
Spec: `.ina/specs/20260420-2012-think-workspace-indexer-v2-hardening.md`

- [x] **T1** `store.DeleteWorkspaceFilesByPrefix` — range query(`abs_path` BINARY) + FTS sync + trailing-slash 정규화. 정상 경로 테스트.
- [x] **T2** `store.DeleteWorkspaceFilesByPrefix` — LIKE 메타문자 안전성 + exact 파일 경로 regression.
- [x] **T3** `FTS5Indexer.RemoveFile` — prefix 호출로 전환. dir/file 양쪽 멱등.
- [x] **T4** `Watcher.partialAdds` atomic.Int64 + `PartialAddFailures()` + 3곳 Debug→Warn 승격 (`phase` 키: `initial_walk` / `initial_subdir` / `runtime_create`).
- [x] **T5** `LiveIndexer.PartialFailures()` delegate + Close-after 호출 안전성 테스트.
- [x] **T6** E2E wiring canary: tempdir dir Remove → mockIndexer가 dir path로 `RemoveFile` 수신.
- [x] **T7** 문서 정리 — CLAUDE.md "Known limitations" 2개 삭제 (IN_Q_OVERFLOW만 잔존), TASKS.md Archive 이동.

### 🔴 잔존: Workspace Indexer v2 Hardening (별도 PR)

- [ ] **Queue overflow 복구**: Linux `IN_Q_OVERFLOW` 이벤트 감지 시 해당 workspace 자동 full reindex 1 회 (debounce + 중복 방지). Linux 전용이라 분리.
- [ ] **Dead code 정리**: `store.DeleteWorkspaceFileByAbsPath` — 1·2 번들 이후 호출처 0개. 사용처 확인 후 제거.

### 🟡 중간 우선순위

- [ ] **MoA (Mixture of Agents)** — 복수 모델 병렬 질의 + 기본 모델 합성. config `[[models]]` 전체 fan-out. KittyPaw Rust 원본 `skill_executor/moa.rs` 대응 (~100 줄 수준).
- [ ] **File.summary(path) + LLM 캐시** — 파일 LLM 요약 + 캐시. Agent workspace 이해 품질 ↑.
