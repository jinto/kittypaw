...
Tokens stored **per-tenant** in `~/.kittypaw/tenants/<tenantID>/secrets.json` under namespace `kittypaw-api/{host}` (e.g. `kittypaw-api/localhost:8080`). All CLI flows (`kittypaw login`, `kittypaw setup`, Kakao wizard) write to the default tenant's store via `core.LoadTenantSecrets("default")`; the daemon reads from the same per-tenant path through `tenant_deps.go`. The earlier "OAuth-once-per-host" global model (commit `c1a0c58`) was retired so each tenant on a shared host can have its own credentials — multi-user UX (`--user` flag) lands in a follow-up plan. `APITokenManager` (`core/api_token.go`) handles auto-refresh with single-flight mutex pattern. JWT expiry checked client-side via base64 payload decode (no signature verification) with 30-second grace window.
...
KakaoTalk relay pairing skips OAuth entirely: `wizardKakao` (CLI) and `handleSetupKakaoRegister` (web) hit `/discovery` anonymously, `POST {relay_url}/register` directly, then persist the full `wss://…/ws/{token}` under `kittypaw-api/{host}` in the same per-tenant store. `InjectKakaoWSURL(tenantID, channels)` (invoked by `ChannelSpawner.Reconcile` as the single injection site) opens the named tenant's secrets via `LoadTenantSecrets`, reads the saved relay URL, and populates `KakaoWSURL` on the runtime channel config — the token is never written to `config.toml`, so rotations don't require a config edit. The `Reconcile` signature itself is unchanged (load-bearing sync contract per `TestHandleReload_WaitsForReconcile`); only `InjectKakaoWSURL` learned the tenant context.
