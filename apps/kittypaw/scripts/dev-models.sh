...
write_config_if_missing() {
  ...
  ensure_binary
  # Run the setup wizard non-interactively to create auth.json (Web UI
  # credentials, required for chat WS handshake) + account.toml +
  # secrets.json. The provider/model values here are placeholders — the
  # cat block below overwrites config.toml with the 4-model template.
  # --password-stdin uses a throwaway "devpw"; the daemon is loopback-only
  # and the dir is /tmp, so this does not cross any trust boundary.
...
bind = "$TEST_BIND"
master_api_key = "dev-models-master-key"
TOML
