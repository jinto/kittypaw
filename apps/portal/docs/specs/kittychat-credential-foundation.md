...
kittychat 은 *우리 발급 credential 의 검증자*. `apps/portal` 이 *발급자 + contract source of truth*. 분산 client (daemon) 환경 → wire protocol **additive only**.
...
- **`aud` 정책 = multi-aud** (사용자 결정 2026-05-02): `aud=["https://api.kittypaw.app","https://chat.kittypaw.app"]` — RFC 7519 standard, 단일 token, BC 안전.
...
- API client JWT: 두 audience 모두 (kittypaw SDK 가 API resource 호출 + kittychat 가 동일 token 검증)
- daemon JWT: `aud=["https://chat.kittypaw.app"]` 만 — daemon 은 API resource 직접 호출 안 함
...
- **RS256** — portal 발급, kittychat 이 JWKS public key 로 검증. HS256 secret 공유 risk 제거.
...
- process 추출 완료: `apps/portal`이 별도 deployable service로 auth/discovery/JWKS를 소유한다.

**책임 분리 명시**:
| 역할 | host (지금) | path | 책임 |
|---|---|---|---|
| auth authority | `portal.kittypaw.app` | `/auth/*` | OAuth login, refresh, /me, JWT 발급 |
| api resource server | `api.kittypaw.app` | `/v1/*` | 데이터 프록시 |
| chat resource server | `chat.kittypaw.app` | `/*` | relay (검증만) |

`apps/portal`과 `apps/kittyapi`는 별도 process다. Production DB 물리는 아직 기존 DB를 공유하며, 물리 DB 분리는 별도 cutover에서 다룬다.
...
- **T3**: OAuth Google/GitHub callback 의 access token 발급 시 default claims 적용 (`aud=["https://api.kittypaw.app","https://chat.kittypaw.app"]`, `scope=["chat:relay","models:read"]`, `v=2`)
...
- [ ] *신규* 발급 token 이 multi-aud + scope + v=2 박제 검증 — `TestSignForAudiences_*`
