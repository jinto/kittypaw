...
- **JWT + 리프레시 토큰** — 15분 액세스, 7일 리프레시 (로테이션 + 재사용 탐지). 발급 토큰에 `aud=["kittyapi","kittychat"]`, `scope`, `v=1` claim 포함 — [`docs/specs/kittychat-credential-foundation.md`](docs/specs/kittychat-credential-foundation.md) 참고.
- **속도 제한** — IP 기반 비인증 (5/분) + 사용자 기반 인증 (60/분), 일일 10K 상한
- **서비스 디스커버리** — `GET /discovery`로 카카오톡 릴레이, API, 스킬 레지스트리 URL 반환
...
|---|---|---|
| `GET` | `/health` | 헬스 체크 |
| `GET` | `/discovery` | 서비스 URL (카카오톡 릴레이, API 베이스, 스킬 레지스트리) |
