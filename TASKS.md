...
- **`tenant add` interactive fallback** — `kittypaw tenant add <name>` (no flags) 시 4 단계 prompt (telegram token / LLM provider / api-key / model) 진입. Secrets 는 `term.ReadPassword` 로 echo-off (TTY only). `needsTenantPrompt` gating + `*os.File` type assert 로 test fallback path. CI / scripted (flag/env 있음) 호환성 그대로. `cc7dd7e`
