...
## Multi-user Blockers (A묶음) ⚠️ MB2/MB3 reverted by Z1 (2026-04-20)
...
- [~] MB2. ~~spawnerKey.Alias + ChannelConfig.Alias~~ — **Z1 revert**: tenant == user 모델 확정 (household은 multi-tenant + family coordinator로). spawnerKey는 `{TenantID, ChannelType}`, 테넌트당 타입별 단일 채널.
- [~] MB3. ~~ChatPayload.FromID~~ — **Z1 revert**: 실제 소비자 없음. per-user 세션 구분은 SessionID로 충분. FromName은 package context injection(`user_name`)용으로 유지.
...
- MB2/MB3 revert → `refactor(core): remove speculative multi-user-within-tenant scaffolding` (Z1, 2026-04-20)
